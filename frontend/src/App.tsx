...
import { useGraph, useCreateConcept } from "./api/queries";
...
function TopBar({ onNewConcept, pending }: { onNewConcept: () => void; pending: boolean }) {
  return (
    <header className="flex h-12 shrink-0 items-center justify-between border-b border-gray-200 bg-white px-4">
      <span className="text-sm font-semibold text-gray-900">Visual Brainstorming Graph</span>
      <Button onClick={onNewConcept} disabled={pending}>
        New concept
      </Button>
    </header>
  );
...
function AppShell() {
  ...
  return (
    <div className="flex h-screen w-screen flex-col">
      <TopBar onNewConcept={handleNewConcept} pending={createConcept.isPending} />
