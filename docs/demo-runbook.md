...
4. **Live generation.** Drag the "+" under the best branch result onto empty canvas. A pre-filled draft appears. Drag the "+" under the busy-wall candidate onto that draft (it shows a dashed outline, then "Add as parent" under the pointer): a second parent edge appears. In chat type "Bring in this one's composition." and press Enter, then click Generate. Narrate while it runs (about a minute): the concept is frozen, the prompt is compiled once, both parent images go to the image model as references, the dislike box on the wall is folded into the prompt.
